...
public class TESRIndustrialBatteryBank extends TESRBase<BlockEntityIndustrialBatteryBank>{

    private static final ItemStack lBattery = new ItemStack(ModItems.BATTERY_LITHIUM.get());

    public TESRIndustrialBatteryBank(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(BlockEntityIndustrialBatteryBank blockEntity, float partialTick, PoseStack stack, MultiBufferSource buffer, int combinedOverlay, int packedLight) {
        ...
        if (blockEntity != null && blockEntity.isMaster())
        {
            ...
            if (blockEntity!=null && blockEntity.isMaster() && blockEntity.isBase())
            {
                ...
                renderPointer(stack, lighting(blockEntity), combinedOverlay, buffer, facing, xPos, y + 0.486, zPos, blockEntity.getInPutAngle(), pointerLong, 0.6F);
                doTheMath(facing, x, z, 1.97, -0.72);
                renderText(stack, facing, xPos, y + 0.21, zPos, blockEntity.getInPutText(), 0.008F);
                ...
                renderText(stack, facing, xPos, y + 0.984f, zPos, blockEntity.getInPutIndicatorText(), 0.008F);

                doTheMath(facing, x, z, 1.97, 0);
                renderText(stack, facing, xPos, y + 1.0, zPos, blockEntity.getEnergyText(), 0.006F);
                renderBarLevel(stack, lighting(blockEntity), combinedOverlay, buffer, facing, xPos, y + 1.14, zPos, blockEntity.getBatteryFill(), 1.2F);

                doTheMath(facing, x, z, 1.97, 0.846f);
                renderPointer(stack, lighting(blockEntity), combinedOverlay, buffer, facing, xPos, y + 0.486, zPos, blockEntity.getOutPutAngle(), pointerLong, 0.6F);
                doTheMath(facing, x, z, 1.97, 0.72f);
                renderText(stack, facing, xPos, y + 0.21, zPos, blockEntity.getOutPutText(), 0.008F);
                render3dItem(stack, lighting(blockEntity), combinedOverlay, buffer, facing, blockEntity.getLevel(), xPos, y + 0.96f, zPos, label_5, 1.6f, false);
                renderText(stack, facing, xPos, y + 0.984f, zPos, blockEntity.getOutPutIndicatorText(), 0.008F);
            }
